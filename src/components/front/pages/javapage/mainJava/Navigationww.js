...
import '../../webpage/mainWeb/Navigationww.css';
  ...
  return (
    <nav className={`nav ${isMenuOpen ? 'open' : ''} ${isScrolled ? 'scrolled' : ''}`}>
      ...
      <ul className={`nav-links ${isMenuOpen ? 'open' : ''}`}>
        ...
        <li><a href="#course" onClick={toggleMenu}>{language === 'UA' ? 'ПРО КУРСИ' : 'ABOUT COURSES'}</a></li>
        <li><a href="#proces" onClick={toggleMenu}>{language === 'UA' ? 'ПРОЦЕС НАВЧАННЯ' : 'LEARNING PROCESS'}</a></li>
        <li><a href="#course-structure" onClick={toggleMenu}>{language === 'UA' ? 'МОДУЛІ' : 'MODULES'}</a></li>
